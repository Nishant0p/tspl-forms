  ...
  return (
    <div
      ...
      className={cn(
        ...
        !isSelected && 'hover:shadow-md cursor-pointer hover:border-foreground/30 border-l-[4px] border-l-border hover:border-l-foreground/50',
        isSelected &&
          'border-l-[5px] border-l-foreground dark:border-l-foreground shadow-lg ring-1 ring-foreground/15'
      )}
      onClick={(e) => {
        e.stopPropagation();
        if (!isSelected) {
    ...
    >
      ...
      {topHalf.isOver && (
        <div className="absolute top-0 h-[4px] w-full rounded-t-md bg-foreground z-30" />
      )}
      {bottomHalf.isOver && (
        <div className="absolute bottom-0 h-[4px] w-full rounded-b-md bg-foreground z-30" />
      )}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex-1 w-full space-y-1">
              <Input
                ...
                className="h-12 text-base font-normal bg-[#f8f9fa] dark:bg-muted/30 border-b-2 border-t-0 border-x-0 rounded-none focus-visible:ring-0 focus-visible:border-foreground px-3 transition-colors placeholder:text-muted-foreground/60 shadow-none"
              />
            </div>
                ...
                {optionsList.map((opt: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-3">
                    ...
                    <Input
                      ...
                      className="h-9 text-sm flex-1 bg-transparent border-b border-t-0 border-x-0 rounded-none focus-visible:ring-0 focus-visible:border-foreground px-1 placeholder:text-muted-foreground/60"
